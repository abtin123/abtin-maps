...
def write_records(path: Path, records: Iterable[dict[str, Any]], index_path: Path | None = None) -> int:
    """Write the existing ABMV1 stream and, optionally, its spatial index.

    ABMV1 itself is unchanged.  When ``index_path`` is supplied the builder
    records byte offsets while writing so the app can seek to viewport records
    instead of scanning the entire country vector member.
    """
    count = 0
    record_offsets: list[tuple[int, int]] = []
    materialized = list(records) if index_path is not None else records
    with path.open("wb") as f:
        ...
        for record in materialized:
            ...
            record_offsets.append((offset, 4 + len(payload)))
            count += 1
    if index_path is not None:
        from .spatial_index import write_index
        write_index(index_path, materialized, record_offsets)
    return count
