...
def write_spatial_chunks(
    ...
    """Write compressed-friendly spatial chunks and a compact layer index.

    The ABM format remains the same record stream (ABMV1). The only change is
    packaging: records are grouped into spatially-local chunks. Each chunk is
    an ordinary ABM vector stream, so ZIP can still DEFLATE it. The index lets
    the mobile reader select only intersecting chunks instead of scanning the
    complete country vector member.
    """
    items = []
    for rec in records:
        bbox = feature_bbox(rec)
        if bbox is not None:
            items.append((bbox, rec))
    # A fixed global grid gives stable spatial locality without duplicating
    # features that cross cell boundaries. Features are sorted by cell first,
    # then by bbox, so neighbouring features tend to share chunks.
    def key(item):
        b = item[0]
        cx = (b[0] + b[2]) * 0.5
        cy = (b[1] + b[3]) * 0.5
        gx = int(math.floor((cx + 180.0) / 360.0 * 256.0))
        gy = int(math.floor((cy + 90.0) / 180.0 * 128.0))
        return gx, gy, b[1], b[0], b[3], b[2]
    items.sort(key=key)
    ...
    if not items:
        ...
        path = out_dir / name
        write_records(path, [])
        chunks.append({"path": f"vector/{layer}/{name}", "bbox": [180.0, 90.0, -180.0, -90.0], "count": 0})
    for start in range(0, len(items), max(1, chunk_size)):
        batch = items[start:start + max(1, chunk_size)]
        name = f"{start // max(1, chunk_size):06d}.bin"
        path = out_dir / name
    ...
    return {"version": 1, "layer": layer, "count": len(items), "chunks": chunks}
