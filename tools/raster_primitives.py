...
class TileStore:
    ...
    def draw_line(self, coordinates, color, width, min_zoom=None):
        ...
        for z in range(first_zoom, self.max_zoom + 1):
            # عرضِ خط قبلاً برای همهٔ زوم‌ها یکسان بود (مثلاً بزرگراه همیشه ۱۰px)،
            # یعنی همان تعداد پیکسل روی یک ناحیهٔ جغرافیاییِ خیلی بزرگ‌تر در زوم
            # پایین کشیده می‌شد و جاده‌ها موقع دور شدن غیرطبیعی کلفت به‌نظر
            # می‌رسیدند (برخلاف نقشه‌های مرجع مثل Magic Earth که هر چه دورتر،
            # خط نازک‌تر می‌شود). این تابع اکنون عرض را نسبت به فاصلهٔ این زوم تا
            # زوم حداکثرِ ساختِ همین عارضه با نصف‌شدنِ تدریجی (هر ۲ زوم یک واحد کم)
            # مقیاس می‌کند و کف ۱px را برای خوانایی حفظ می‌کند.
            zoom_scale = max(0.35, 1.0 - (self.max_zoom - z) * 0.09)
            scaled_width = max(1, round(width * zoom_scale))
