...
class TileStore:
    def __init__(self, root, min_zoom, max_zoom):
        self.root = Path(root)
    ...
    def image(self, z, x, y, layer="tiles"):
        ...
        self.open[key] = image
        if len(self.open) > MAX_OPEN_TILES:
    ...
    def draw_line(self, coordinates, color, width):
    ...
    def finalize_webp(self):
        self.flush()
        count = 0
        for png in self.root.rglob("*.png"):
...
class RoadHandler(osmium.SimpleHandler):
    ...
    def way(self, way):
        ...
        if len(coords) > 1:
            ...
            self.store.draw_line(coords, "#1C2530", width + 2)
            self.store.draw_line(coords, color, width)
