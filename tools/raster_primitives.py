...
TILE_SIZE = 512
MAX_OPEN_TILES = 96
WIDTH_ZOOM_REFERENCE = 14  # عرض جاده در ROAD_STYLE مربوط به همین زوم مطلق است
WIDTH_ZOOM_BASE = 1.32     # هر زوم دورتر: عرض / ۱.۳۲ ، هر زوم نزدیک‌تر: عرض × ۱.۳۲
MIN_POLYGON_PIXELS = 2.5   # پلیگون‌هایی که bbox‌شان در این زوم از این کوچک‌تر است رسم نمی‌شوند
ROAD_STYLE = {
...
def _clip_polygon(points, ox, oy, size):
    """برش Sutherland-Hodgman چندضلعی به مربعِ یک کاشی (به‌علاوه یک پیکسل
    حاشیه). فقط برای عارضه‌های بزرگ (رودخانه/محدوده) به کار می‌رود تا رسم روی
    هر کاشی به تعداد نقاطِ واقعاً داخلِ آن کاشی محدود شود، نه کل عارضه."""
    x0, y0, x1, y1 = ox - 1, oy - 1, ox + size + 1, oy + size + 1
    ...
    def intersect_x(p0, p1, x):
        px0, py0 = p0
        px1, py1 = p1
        if px1 == px0:
            return (x, py0)
        t = (x - px0) / (px1 - px0)
        return (x, py0 + t * (py1 - py0))

    def intersect_y(p0, p1, y):
        px0, py0 = p0
        px1, py1 = p1
        if py1 == py0:
            return (px0, y)
        t = (y - py0) / (py1 - py0)
        return (px0 + t * (px1 - px0), y)

    poly = list(points)
    poly = clip_edge(poly, lambda p: p[0] >= x0, lambda a, b: intersect_x(a, b, x0))
    if not poly:
        return []
    poly = clip_edge(poly, lambda p: p[0] <= x1, lambda a, b: intersect_x(a, b, x1))
    if not poly:
        return []
    poly = clip_edge(poly, lambda p: p[1] >= y0, lambda a, b: intersect_y(a, b, y0))
    if not poly:
        return []
    poly = clip_edge(poly, lambda p: p[1] <= y1, lambda a, b: intersect_y(a, b, y1))
    return poly
...
class TileStore:
    ...
    @staticmethod
    def _tiles_for_segment(p0, p1, z):
        """کاشی‌هایی که یک پاره‌خط از نقطه p0 به p1 را لمس می‌کند (برای کران‌دار
        کردن رسم به کاشی‌های واقعاً درگیر، نه کل bbox عارضه)."""
        x0, y0 = int(p0[0] // TILE_SIZE), int(p0[1] // TILE_SIZE)
        x1, y1 = int(p1[0] // TILE_SIZE), int(p1[1] // TILE_SIZE)
        lo_x, hi_x = min(x0, x1), max(x0, x1)
        lo_y, hi_y = min(y0, y1), max(y0, y1)
        limit = (1 << z) - 1
        for x in range(max(0, lo_x), min(limit, hi_x) + 1):
            for y in range(max(0, lo_y), min(limit, hi_y) + 1):
                yield x, y
    ...
    def draw_line(self, coordinates, color, width, min_zoom=None):
        ...
        for z in range(first_zoom, self.max_zoom + 1):
            # عرض اکنون نسبت به زوم مطلق مقیاس می‌گیرد (نه max_zoom این build)،
            # یعنی یک بزرگراه در z10 در همهٔ کشورها/نسخه‌ها عرض یکسان دارد، صرف‌نظر
            # از اینکه atlas تا کجا رندر شده. پایه: z14 = عرض کامل تعریف‌شده در
            # ROAD_STYLE؛ هر زوم دورتر با ضریب ۱.۳۲ نازک‌تر می‌شود، کف ۱px.
            scaled_width = max(1, round(width * (WIDTH_ZOOM_BASE ** (z - WIDTH_ZOOM_REFERENCE))))
            points = [mercator(lon, lat, z) for lon, lat in coordinates]
            # قبلاً هر عارضه با کل نقاطش روی هر کاشیِ داخل bbox رسم می‌شد؛ برای
            # راه‌های طولانی (مثل بزرگراه‌های سراسری ایران) این یعنی رسم کل
            # مسیر روی صدها کاشی در هر زوم -> هزینه O(کاشی×نقطه). این تابع
            # اکنون هر segment را فقط روی کاشی‌های خودش، با مختصاتِ محلیِ همان
            # segment، رسم می‌کند: هزینه به O(طول مسیر) نزدیک می‌شود.
            by_tile = {}
            for p0, p1 in zip(points, points[1:]):
                for x, y in self._tiles_for_segment(p0, p1, z):
                    by_tile.setdefault((x, y), []).append((p0, p1))
            for (x, y), segments in by_tile.items():
                ox, oy = x * TILE_SIZE, y * TILE_SIZE
                draw = ImageDraw.Draw(self.image(z, x, y))
                for p0, p1 in segments:
                    local = [(p0[0] - ox, p0[1] - oy), (p1[0] - ox, p1[1] - oy)]
                    draw.line(local, fill=color, width=scaled_width, joint="curve")
    ...
    def draw_polygon(self, coordinates, fill, outline=None, extrusion=0):
        ...
        for z in range(self.min_zoom, self.max_zoom + 1):
            ...
            # عارضه‌هایی که در این زوم چند پیکسل هم نمی‌شوند (مثلاً یک ساختمان
            # در زوم خیلی دور) نه دیده می‌شوند و نه ارزش رسم دارند؛ رسم‌شان فقط
            # نویز/بلور اضافه و I/O بی‌فایده روی کاشی می‌سازد.
            px_w = max(x for x, _ in points) - min(x for x, _ in points)
            px_h = max(y for _, y in points) - min(y for _, y in points)
            if px_w < MIN_POLYGON_PIXELS and px_h < MIN_POLYGON_PIXELS:
                continue
            tile_span = (max_x - min_x + 1) * (max_y - min_y + 1)
            # عارضه‌های کوچک (اکثریت راه/چندضلعی) رفتار قبلی را حفظ می‌کنند.
            # وقتی یک عارضه (رودخانه/محدوده‌ی بزرگ) ده‌ها یا صدها کاشی را در بر
            # می‌گیرد، رسمِ کل نقاطِ آن روی هر کاشی هزینه‌ی O(کاشی×نقطه) می‌سازد؛
            # این حالتِ واقعیِ کندی روی کشورهای بزرگ مثل ایران در زوم‌های بالا بود.
            # به‌جای رسمِ دقیقِ لبه در این حالت، هر کاشیِ داخلِ bbox که کاملاً
            # درون چندضلعی است را یکپارچه پر می‌کنیم و فقط کاشی‌های لبه را با
            # نسخه‌ی برش‌خورده‌ی نقاط رسم می‌کنیم.
            for x in range(min_x, max_x + 1):
                for y in range(min_y, max_y + 1):
                    ox, oy = x * TILE_SIZE, y * TILE_SIZE
                    if tile_span > 64:
                        clipped = _clip_polygon(points, ox, oy, TILE_SIZE)
                        if not clipped:
                            continue
                        local = [(px - ox, py - oy) for px, py in clipped]
                    else:
    ...
    def finalize_webp(self, quality=64, method=3, workers=1):
        ...
        if not jobs:
            return 0
        print(
            f"WebP conversion: {len(jobs):,} tiles | workers={workers} | "
            f"quality={quality} | method={method}",
            flush=True,
        )
        if workers <= 1:
            for completed, job in enumerate(jobs, start=1):
                _encode_webp(job)
                if completed == len(jobs) or completed % 1024 == 0:
                    print(f"WebP progress: {completed:,}/{len(jobs):,}", flush=True)
        else:
            with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
                for completed, _ in enumerate(
                    executor.map(_encode_webp, jobs, chunksize=64), start=1
                ):
                    if completed == len(jobs) or completed % 1024 == 0:
                        print(f"WebP progress: {completed:,}/{len(jobs):,}", flush=True)
        return len(jobs)
...
class RoadHandler(osmium.SimpleHandler):
    ...
    def way(self, way):
        ...
        if len(coords) > 1:
            ...
            min_zoom = {
                "motorway": 3, "trunk": 5, "primary": 7,
                "secondary": 9, "tertiary": 10, "residential": 12,
                "unclassified": 12, "service": 13,
            }.get(kind, 13)
...
def main():
    ...
    # z14 نمایش خیابان و تقاطع را ممکن می‌کند؛ اپ برای مسیریابی خیابانی روی
    # همین کاشی overzoom می‌کند (نگاه کنید overzoom_max)، نه رندر z16+.
    ...
    try:
        ...
        metadata = {
            ...
            "overzoom_max": min(args.max_zoom + 5, 19),
