...
def build_sharded_atlas(
    *, pbf: Path, profile: dict[str, Any], code: str, out_dir: Path, work_dir: Path,
    ...
    """نسخهٔ موازیِ ساختِ atlas برای کشورهای خیلی بزرگ (ایران و مشابه): pbf به
    ۴ ربعِ جغرافیایی برش می‌خورد (osmium extract)، هر ربع هم‌زمان با یک
    rendered_tile_builder.py جدا رندر می‌شود (--shard-out)، و در آخر همه‌ی
    کاشی‌ها در یک atlas واحد ادغام می‌شوند (--merge-shards). تنها گلوگاهِ واقعیِ
    build کشورِ بزرگ، پاسِ تک‌رشته‌ایِ osmium روی کل pbf بود؛ این تابع همان پاس
    را بین ۴ پردازشِ موازی (هرکدام تقریباً یک‌چهارمِ کار) تقسیم می‌کند."""
    ...
    print(f"+ رندر موازیِ {len(quadrants)} تکه: {', '.join(quadrants)}", flush=True)
    if not dry_run:
