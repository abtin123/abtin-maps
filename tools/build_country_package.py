...
def main() -> int:
    ...
    if not args.skip_atlas:
        atlas_command = [
            ...
            # ۵۱۲ برای کشورهای کوچک کافی بود، اما برای کشورهای بزرگ (ایران و
            # مشابه) در زوم‌های بالا یک راه یا رودخانه می‌تواند هزاران کاشی را
            # لمس کند؛ سقف کم یعنی flush/reload مکررِ همان کاشی روی دیسک که
            # ساخت را چندبرابر کند می‌کرد (علت واقعی timeout در CI). ۴۰۹۶
            # پیش‌فرضِ امن‌تری است و هر کشور می‌تواند در countries.json مقدار
            # بزرگ‌تر (مثلاً برای ایران/روسیه/چین) تعیین کند.
