...
POI_CLASSES = {
    ...
    "speed_camera": 70, "traffic_signals": 72,
}
...
def resolve_building_height_dm(tags, opts):
    if opts.building_height_mode == "simple":
        return round(opts.building_default_height_m * 10)
    height = parse_width_m(tags.get("height"))
    if height is None:
        try:
            levels = float((tags.get("building:levels") or "").split(";")[0])
            height = levels * 3.1
        except ValueError:
            height = opts.building_default_height_m
    return round(max(2.5, min(opts.building_max_height_m, height)) * 10)
...
class Extractor(osmium.SimpleHandler):
    ...
    def way(self, w):
        ...
        area_klass, area_zoom, area_attr = None, 15, 0
        if t.get("natural") in ("water", "bay") or t.get("waterway") == "riverbank" \
            ...
            area_klass, area_zoom = KLASS_BUILDING, self.o.building_min_zoom
            area_attr = resolve_building_height_dm(t, self.o)
        if area_klass is not None and closed:
            self.stats["areas"] += 1
            self.sink.emit_area(pts, area_klass, area_attr, area_zoom, name)
            return
...
def main(argv=None):
    ...
    ap.add_argument("--poi-min-zoom", type=int, default=12)
    ap.add_argument("--drop-buildings", action="store_true")
    ap.add_argument("--building-min-zoom", type=int, default=15)
    ap.add_argument("--building-height-mode", choices=["simple", "osm"], default="simple")
    ap.add_argument("--building-default-height-m", type=float, default=5.0)
    ap.add_argument("--building-max-height-m", type=float, default=80.0)
    ap.add_argument("--profile", choices=["compact", "balanced"], default="compact")
    ...
    o.overview_tol = [
        (3200.0 if z <= 4 else (900.0 if z <= 6 else 160.0))
        if o.profile == "compact"
        else (2500.0 if z <= 4 else (700.0 if z <= 6 else 120.0))
        for z in o.overview_zooms
    ]
    o.base_tol = 4.0 if o.profile == "compact" else 3.0
    build(o)
    return 0
