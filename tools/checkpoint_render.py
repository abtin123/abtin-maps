...
def source_fingerprint(url: str) -> dict[str, str]:
    ...
    try:
        ...
        # ادامه دادن با checkpoint بدون هویت منبع خطرناک است؛ build باید در
        # اجرای بعدی و پس از رفع مشکل شبکه، با دادهٔ معتبر آغاز شود.
...
def render_stage(
    *, profile: dict[str, Any], code: str, stage_name: str, bounds: list[float],
    ...
    # یک خروجی ناقص هرگز نباید با checkpoint سالم اشتباه گرفته شود.
    ...
    try:
        ...
        command = [
            ...
            # دقیقاً یک renderer و یک worker WebP برای هر stage.
            "--workers", "1",
...
def main() -> int:
    ...
    try:
        ...
        # PBF در cache همان job می‌ماند؛ در اجرای بدون cache هم با پایان job حذف می‌شود.
