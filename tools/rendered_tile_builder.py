...
def build_atlas(root, output, code, minimum, maximum, bounds, target_mb):
    ...
    db.executemany("INSERT INTO metadata VALUES (?,?)", {
        ...
        # اپ برای ناوبری خیابانی تا این زوم روی آخرین کاشیِ واقعی (z=max_zoom)
        # overzoom (بزرگ‌نمایی کلاینتی، بدون کاشی جدید) می‌کند تا صفحه خالی
        # نشود؛ سقف ۱۹ چون بالاتر از آن overzoom عملاً فقط پیکسل‌های کور بزرگ‌شده است.
...
def main():
    ...
    # z2: کل کشور در یک نگاه (overview). اگر بودجهٔ اندازه کم باشد
    # select_max_zoom سطح پایدارِ مناسب زیرِ این سقف را انتخاب می‌کند.
    parser.add_argument("--min-zoom", type=int, default=2)
    # z14 جزئیات خیابانی قابل‌خواندن می‌دهد؛ زوم‌های بالاتر با overzoom
    # کلاینتی روی همین کاشی پوشش داده می‌شوند (metadata.overzoom_max).
    ...
    if args.merge_shards:
        # حالت «ادغام»: هیچ رندری انجام نمی‌شود، فقط کاشی‌های WebP از چند
        # اجرای موازیِ --shard-out قبلی در یک درخت واحد جمع و atlas نهایی از
        # روی آن ساخته می‌شود.
    ...
    try:
        ...
        if args.shard_out:
            # حالت «تکه»: فقط کاشی‌های WebP همین بخش جغرافیایی را بیرون
            # می‌گذاریم؛ atlas نهایی بعداً در حالت --merge-shards ساخته می‌شود.
        ...
        # اگر --shard-out قبلاً work را move کرده باشد، این دیگر چیزی برای پاک
        # کردن پیدا نمی‌کند و بی‌خطر عبور می‌کند.
        shutil.rmtree(work, ignore_errors=True)
