...
def create_abm(staging: Path, output: Path, country: str, source: Path, stats: dict,
    ...
    metadata={
        ...
        "style": "external",
        "renderer": "app",
        "vector_index": "ABMIDX1",
        "vector_index_cell_degrees": 0.5,
        "search": True,
        "routing": True,
        "source": source.name,
    ...
    with zipfile.ZipFile(output,'w',zipfile.ZIP_DEFLATED,compresslevel=6) as z:
        for p in sorted(staging.rglob('*')):
            ...
            # Indexed vector members must be seekable.  They remain the same
            # ABMV1 stream inside the ABM container; only the outer ZIP entry
            # uses STORE instead of DEFLATE.  The index is tiny and is also
            # stored so the app can open it without decoding any other ZIP
            # member. SQLite/metadata remain compressed to keep the archive compact.
            if arcname.startswith('vector/') and (arcname.endswith('.bin') or arcname.endswith('.idx')):
