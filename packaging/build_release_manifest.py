...
def main():
    ...
    ap.add_argument('--previous-manifest', type=Path, default=None,
                     help='Previously published manifest.json, if any. Its entries are kept '
                          'for any country/region code NOT rebuilt in this run (e.g. a '
                          'workflow_dispatch that only rebuilt one country), so a partial '
                          'run never drops every other already-published map from the '
                          'manifest - only the codes actually present in `dist` are updated.')
    ...
    # Start from whatever was already published (keyed by code) so a partial
    # rebuild only overwrites the codes it actually touched.
    ...
    manifest = {
        ...
        'countries': [merged[code] for code in sorted(merged)],
    }
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print(f'Wrote {args.output} with {len(manifest["countries"])} entries '
          f'({len(codes)} rebuilt this run, {len(manifest["countries"]) - len(codes)} carried over)')
