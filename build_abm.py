...
def main():
    ...
    ap.add_argument('--regions', type=Path, default=None,
                     help='Optional JSON config splitting a large country into real '
                          'geographic regions (feature-selected by bbox), instead of one whole-country '
    ...
    try:
        for region_def in cfg['regions']:
            ...
            routing_bbox = (
                max(-180.0, bbox[0] - overlap),
                max(-90.0, bbox[1] - overlap),
                min(180.0, bbox[2] + overlap),
                min(90.0, bbox[3] + overlap),
            )
            routing_dataset = (
                None if is_pbf else clip_dataset(data, routing_bbox)
            )
            region_meta = {
                'code': code,
