...
def _build_one(dataset, country: str, output: Path, source: Path, region: dict | None = None) -> Path:
    ...
    try:
        ...
        # The routing builder already streams PBF, so retaining the fully
        # materialized Dataset here only wastes RAM during the most expensive
        # phase. Release the source object graph before routing.
