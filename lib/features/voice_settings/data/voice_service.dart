...
class VoiceService {
  ...
  Future<void> _initTts() async {
    try {
      ...
      final modelFile = File('${appDocDir.path}/fa_vits.onnx');
      
      if (!await modelFile.exists()) {
        // Check if assets exist
        try {
          await rootBundle.load('assets/models/tts/fa_vits.onnx');
        } catch (_) {
          _isModelMissing = true;
          print('TTS Models not found in assets or local storage.');
          return;
        }
      }
  ...
  Future<void> downloadModels() async {
    ...
    try {
      final appDocDir = await getApplicationDocumentsDirectory();
      final baseUrl = 'https://github.com/k2-fsa/sherpa-onnx/releases/download/tts-models';
      
      final files = {
        'fa_vits.onnx': '$baseUrl/vits-piper-fa_IR-amir-medium.onnx',
        'lexicon.txt': '$baseUrl/lexicon.txt',
        'tokens.txt': '$baseUrl/tokens.txt',
      };

      final client = http.Client();
      for (var entry in files.entries) {
        final file = File('${appDocDir.path}/${entry.key}');
        ...
        if (response.statusCode == 200) {
          ...
          throw Exception('Failed to download ${entry.key}: ${response.statusCode}');
        }
      }
  ...
  Future<void> speak(String text) async {
    ...
    try {
      final generatedAudio = _tts!.generate(text: text);
