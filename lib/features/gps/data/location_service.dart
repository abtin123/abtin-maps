...
class LocationService {
  ...
  final _xFilter = _Kalman1D(processNoisePerSecond: 1.5);
  final _yFilter = _Kalman1D(processNoisePerSecond: 1.5);
  ...
  void _onPosition(Position p) {
    ...
    final filteredLatRaw = _yFilter.process(
      p.latitude, 
      posAccuracy / _metersPerDegLat, 
      tsMs,
      extraProcessNoise: dynamicProcessNoise / _metersPerDegLat
    );
    final filteredLngRaw = _xFilter.process(
      p.longitude, 
      posAccuracy / (_metersPerDegLat * math.cos(degToRad(p.latitude))), 
      tsMs,
      extraProcessNoise: dynamicProcessNoise / (_metersPerDegLat * math.cos(degToRad(p.latitude)))
    );

    var filteredLat = filteredLatRaw;
    var filteredLng = filteredLngRaw;

    // Strict Hysteresis: Prevents micro-movements during stops (Google Maps Style)
    if (_lastSentLat != null && _lastSentLng != null) {
      final distMoved = _calculateDistance(
        filteredLat, filteredLng,
        _lastSentLat!, _lastSentLng!,
      );
      
      // If speed is low, we need a significant jump (5m) to move the marker.
      // This eliminates the "dancing" effect completely.
      final moveThreshold = isMoving ? 1.0 : 5.0;
      
      if (distMoved < moveThreshold) {
        filteredLat = _lastSentLat!;
        filteredLng = _lastSentLng!;
      }
    }
