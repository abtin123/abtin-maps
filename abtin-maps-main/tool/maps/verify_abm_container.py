...
SEARCH_ENTRY = "search/places.sqlite"
...
def verify(path: Path, expected_region: str | None) -> dict[str, object]:
    ...
    with path.open("rb") as handle:
        ...
        if int(metadata.get("maxzoom", 16)) > 16:
            fail("PMTiles metadata maxzoom exceeds 16")
        search_entry = next((e for e in entries if isinstance(e, dict) and e.get("path") == "search/places.sqlite"), None)
        if search_entry is None:
            fail("search/places.sqlite entry is missing")
        search_data = read_exact(handle, int(search_entry["offset"]), int(search_entry["length"]), "search index")
        with tempfile.NamedTemporaryFile(suffix=".sqlite") as tmp:
            tmp.write(search_data); tmp.flush()
            try:
                db = sqlite3.connect(tmp.name)
                names = {row[0] for row in db.execute("SELECT name FROM sqlite_master WHERE type='table'")}
                if "places" not in names or "places_fts" not in names:
                    fail("offline search index is missing places/places_fts tables")
                columns = {row[1] for row in db.execute("PRAGMA table_info(places_fts)")}
                required = {"name", "region", "latitude", "longitude"}
                if not required.issubset(columns):
                    fail("offline search index schema does not match the mobile search reader")
                if db.execute("SELECT COUNT(*) FROM places").fetchone()[0] <= 0:
                    fail("offline search index is empty")
                db.close()
            except sqlite3.Error as exc:
                fail(f"offline search index is not valid SQLite: {exc}")
