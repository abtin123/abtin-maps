...
def main() -> None:
    ...
    parser.add_argument("--delta-meta", type=Path)
    ...
    delta_info = None
    if args.delta and args.delta.is_file() and args.delta_meta and args.delta_meta.is_file():
        meta = json.loads(args.delta_meta.read_text(encoding="utf-8"))
        delta_info = {
            "base_sha256": str(meta.get("base_sha256", "")),
            "manifest_file": args.delta_meta.name,
            "bin_file": args.delta.name,
            "size": args.delta.stat().st_size,
            "sha256": file_sha256(args.delta),
        }
