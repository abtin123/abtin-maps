    ...
    for manifest_path in sorted(args.input_root.glob('*/manifest.json')):
        ...
        for item in data.get('countries', []):
            ...
            if archive.is_file():
                ...
                delta = args.input_root / manifest_path.parent.name / f'{code}.delta'
                delta_meta = args.input_root / manifest_path.parent.name / f'{code}.delta.json'
                if delta.is_file(): shutil.copy2(delta, args.output / delta.name)
                if delta_meta.is_file(): shutil.copy2(delta_meta, args.output / delta_meta.name)
                entries[code] = item
    asset_names = set()
    for item in entries.values():
        for f in item.get('files', []):
            if isinstance(f, dict) and f.get('name'): asset_names.add(str(f['name']))
    if len(asset_names) + 2 > 1000:
        raise SystemExit(f'Release would exceed 1000 assets: {len(entries)} map entries plus manifest/report')
    manifest = {'schema_version': 5, 'release_tag': args.release_tag, 'countries': [entries[k] for k in sorted(entries)]}
