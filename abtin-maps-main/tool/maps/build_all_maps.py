        ...
        for number, entry in enumerate(selected, 1):
            ...
            try:
                ...
                if previous_signature == signature and previous.get("sha256"):
                    ...
                    successes.append({"code": code, "archive": archive.name, "skipped": True, "source_signature": signature})
                    continue
                cmd = ["bash", str(builder), code, str(entry["pbf_url"]), str(args.output)]
                if entry.get("bbox"):
                    cmd.append(str(entry["bbox"]))
                last_error = None
                ...
                successes.append({"code": code, "archive": archive.name, "skipped": False, "source_signature": signature})
            except Exception as exc:
