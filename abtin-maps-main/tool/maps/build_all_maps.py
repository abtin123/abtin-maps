...
def source_signature(url: str) -> str:
    """Return a stable remote revision when the provider exposes ETag/Last-Modified."""
    try:
        request = urllib.request.Request(url, method="HEAD", headers={"User-Agent": "abtin-maps-builder/2"})
        with urllib.request.urlopen(request, timeout=60) as response:
            etag = response.headers.get("ETag", "")
            modified = response.headers.get("Last-Modified", "")
            length = response.headers.get("Content-Length", "")
            return "|".join((etag, modified, length))
    except Exception:
        return ""
...
def empty_manifest(path: Path, tag: str) -> None:
    path.write_text(json.dumps({
        "schema_version": 5, "release_tag": tag,
    ...
    with tempfile.TemporaryDirectory(prefix="abtin-map-build-") as temp:
        for number, entry in enumerate(entries, 1):
            ...
            try:
                ...
                current_signature = source_signature(str(entry["pbf_url"]))
                previous_signature = str(previous.get("source", {}).get("revision", "")) if isinstance(previous.get("source"), dict) else ""
                if previous.get("sha256") and archive.name in existing_assets and current_signature and current_signature == previous_signature:
                    print(f"SKIP {code}: OSM source revision unchanged")
                    successes.append({"code": code, "archive": archive.name, "skipped": True, "unchanged": True})
                    continue
                ...
                old_archive = Path(temp) / f"{code}.old.abm"
                delta = args.output / f"{code}.delta"
                if previous.get("sha256") and args.previous:
                    try:
                        if not old_archive.exists():
                            subprocess.run(["gh", "release", "download", args.release_tag, "--pattern", f"{code}.abm", "--dir", str(temp), "--clobber"], check=True, stdout=subprocess.DEVNULL)
                            downloaded = Path(temp) / f"{code}.abm"
                            if downloaded.exists(): downloaded.rename(old_archive)
                        if old_archive.exists():
                            subprocess.run([sys.executable, str(args.root / "tool/maps/build_abm_delta.py"), "--old", str(old_archive), "--new", str(archive), "--output", str(delta)], check=True)
                            if delta.stat().st_size >= archive.stat().st_size:
                                delta.unlink(); delta.with_suffix('.delta.json').unlink(missing_ok=True)
                            else:
                                print(f"DELTA {code}: {delta.stat().st_size} bytes")
                    except Exception as exc:
                        print(f"WARNING {code}: delta generation skipped: {exc}", file=sys.stderr)
                ...
                subprocess.run([
                    ...
                    "--source-revision", current_signature,
                    *( ["--delta", str(delta), "--delta-meta", str(delta.with_suffix('.delta.json'))] if delta.is_file() and delta.with_suffix('.delta.json').is_file() else []),
                ], check=True)
