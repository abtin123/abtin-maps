    ...
    with tempfile.TemporaryDirectory(prefix="abtin-map-build-") as temp:
        for number, entry in enumerate(entries, 1):
            ...
            try:
                ...
                if archive.is_file() and previous.get('sha256') == file_sha256(archive):
                    print(f"SKIP {code}: existing archive matches manifest hash")
                ...
                subprocess.run([sys.executable, str(updater), "--output", str(next_manifest), "--previous", str(manifest), "--release-tag", args.release_tag, "--archive", str(archive), "--code", code, "--name-fa", str(entry.get("name_fa", code)), "--name-en", str(entry.get("name_en", code)), "--bbox", str(entry.get("bbox", "-180,-90,180,90")), "--source-url", str(entry["pbf_url"])], check=True)
