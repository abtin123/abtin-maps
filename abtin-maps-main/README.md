...
```
metadata.json
map.mbtiles   # تایل‌های برداری (MVT/gzip) برای رندر — ساختار normalized: map + images (تایل‌های یکسان فقط یک‌بار)
map.sqlite    # تنها دیتابیس: Search + POI + Routing (هر رکورد فقط یک‌بار)
```

`map.mbtiles` در zip بدون فشرده‌سازی مجدد (STORED) و `map.sqlite` با DEFLATE ذخیره می‌شه.
لایه‌های تایل: `roads, buildings, landuse, water, boundaries` (zoom 0..14، extent 4096).
مکان‌ها (places) و POIها فقط در `map.sqlite` هستن، نه در تایل.

### اسکیمای `map.sqlite` (v5)

`names` (رشته‌ها یک‌بار) ← `features` (poi/place/road، `id=(osm_id<<2)|kind`) و `way_data`؛ `categories` مشترک؛
`spatial` (rtree نقطه‌ها، تنها منبع lat/lon فیچرها)؛ `search_fts` روی `names`؛
`node_data` (مختصات int×1e-7)، `segments` (یک ردیف برای هر قطعه؛ جهت برعکسِ جاده‌های دوطرفه ذخیره نمی‌شه)،
`road_index` (rtree قطعات ۸تایی برای snap؛ جایگزین node_rtree)، `turn_restrictions`.
ویوهای سازگار: `nodes, ways, edges` (هر دو جهت با `UNION ALL`؛ یال برعکس id منفی دارد)، `places, poi`.

Snap: `SELECT seg_from,seg_to FROM road_index WHERE max_lon>=:x-r AND min_lon<=:x+r AND max_lat>=:y-r AND min_lat<=:y+r`
سپس `segments WHERE id BETWEEN seg_from AND seg_to` و مختصات از `nodes`.
...
`.github/workflows/Build-and-publish-ABM-v4-fixed.yml` هر دوشنبه اجرا می‌شه و **همه‌ی
...
### وقتی چیزی عوض نشده، از نو ساخته نمی‌شه

قبل از دانلود PBF، `packaging/check_source.py` فقط یک درخواست HEAD می‌زنه و کشور رو **کامل رد می‌کنه**
(بدون دانلود، بدون build، بدون آپلود مجدد) اگر همه‌ی این‌ها برقرار باشه:

- نسخه‌ی قبلی کشور در `manifest.json` هست و کامله؛
- همه‌ی فایل‌های اون واقعاً (با همون حجم) در release هستن؛
- کد builder (و فایل regions) نسبت به آخرین build تغییر نکرده (`source.builder_hash`)؛
- منبع OSM عوض نشده (ETag، در غیر این صورت Last-Modified، در غیر این صورت SHA-256 فایل PBF).

اگر هرکدام نقض بشه، همون کشور بازسازی می‌شه. `force=true` همیشه بازسازی می‌کنه.
نقشه‌ای که بدون اطلاعات منبع منتشر شده بود، یک‌بار بازسازی می‌شه تا این اطلاعات ثبت بشه.

> **نکته‌ی مهم درباره‌ی پچ:** پچ فعلی تکه‌های ۴ مگابایتی را *بر اساس موقعیت بایت* مقایسه می‌کنه.
> چون `map.sqlite` و `map.mbtiles` هر هفته از صفر ساخته می‌شن، با ~۱٪ تغییر در داده تقریباً همه‌ی صفحه‌ها
> عوض می‌شن (در آزمایش: ۹۲–۹۴٪ صفحه‌ها) و پچ ≈ ۱۰۰٪ فایل کامل می‌شه؛ `prepare_release.py` در این حالت پچ را
> حذف می‌کنه و اپ دانلود کامل می‌کنه. پچ فقط وقتی مفیده که آرشیو بایت‌به‌بایت یکسان یا تقریباً یکسان باشه.
...
build_abm.py
  ...
  └─ routing/graph_builder.py     map.sqlite (nodes/edges/turn restrictions) (گره‌ها، یال‌ها، نام جاده، junction، محدودیت‌های گردش)
...
# Abtin Maps ABM builder (MBTiles + SQLite)

Builds `.abm` country archives for the Abtin Maps offline map engine.
An `.abm` file is a plain zip containing **only** `metadata.json`,
`map.mbtiles` (gzip MVT, normalized `map`+`images`, identical tiles stored once)
and `map.sqlite` (search + POI + routing, every record stored once). See the
Persian section above for the `map.sqlite` v5 schema.
...
`.github/workflows/Build-and-publish-ABM-v4-fixed.yml` runs every Monday and rebuilds **every
...
Before downloading the PBF, `packaging/check_source.py` sends one HEAD request and skips the country
entirely (no download, build or re-upload) when: the previous manifest entry is complete, all its files
still exist in the release with the same size, the builder code / regions config is unchanged
(`source.builder_hash`), and the OSM source is unchanged (ETag, else Last-Modified, else PBF SHA-256).
`force=true` always rebuilds. A map published without source info is rebuilt once to record it.

Note: the current patch compares 4 MiB chunks *by byte position*. Because `map.sqlite` / `map.mbtiles`
are rebuilt from scratch, ~1% of edited data changes ~92-94% of pages (measured), so the patch is ≈100% of
the full file and `prepare_release.py` drops it (the app then downloads the full archive).
...
build_abm.py
  ...
  └─ routing/graph_builder.py     map.sqlite (nodes/edges/turn restrictions) (nodes, edges, road metadata, turn restrictions)
...
## ABM storage / performance
New builds keep the ABM container tile-free. Vector data uses the existing `ABMV1` record stream, split into spatially-local compressed chunks under `vector/<layer>/`; `vector/index.json` maps chunk bounding boxes. ZIP remains DEFLATE-compressed, so the index does not require uncompressed country-sized vector members. Routing is stored as `map.sqlite (nodes/edges/turn restrictions)` with indexed nodes/edges and an optional RTree. The app supports both these new members and legacy `vector/*.bin` / `routing/graph.bin` ABMs.


### معماری دیتابیس نسخه 4

`map.sqlite` تنها دیتابیس داخل ABM است. اطلاعات پایه‌ی قابل اشتراک فقط یک‌بار در آن نگهداری می‌شود: `features` برای موجودیت‌ها و نام‌ها، و `nodes`/`edges`/`turn_restrictions` برای گراف. Search از FTS5/RTree همین دیتابیس استفاده می‌کند و POI یک view روی `features` است؛ بنابراین `search.sqlite`، `poi.sqlite` و `routing/graph.sqlite` در خروجی نهایی تولید نمی‌شوند. `routing/graph.sqlite` فقط در زمان build به‌صورت موقت ساخته و سپس داخل `map.sqlite` merge می‌شود و قبل از package حذف می‌گردد.
