...
def create_abm(staging: Path, output: Path, country: str, source: Path, stats: dict,
    ...
    # DEFLATE, not LZMA: LZMA gives ~5-10% smaller archives but its decoder
    # is single-threaded and CPU-heavy - on a mid-range phone that shows up
    # as a visible stall the moment the app opens the .abm or seeks into a
    # vector chunk / map.sqlite. DEFLATE is 5-10x faster to decode (zlib is
    # NEON-accelerated on Android/ARM) at a small size cost, which is the
    # right trade for an interactive map that must never hitch mid-pan.
    # Per-file compression, not one algorithm for the whole archive:
    #
    # `map.sqlite` is a single large homogeneous file (all of a country's
    # roads/POIs/search index/routing graph in one SQLite database) that is
    # only ever decompressed ONCE, at install time, to a plain file on disk -
    # the app opens it with sqlite3 afterwards and never re-decodes it during
    # panning/zooming. For a file like that, LZMA's large dictionary matters
    # a lot more than its slower decoder: DEFLATE's window is a fixed 32KB,
    # so on a big file full of long-range repetition (the same tag strings,
    # road names, etc. recurring far more than 32KB apart) it simply can't
    # see most of the redundancy LZMA can - that gap grows with file size,
    # which is why a small test fixture understates it badly. So: LZMA here.
    #
    # `vector/**/*.bin` chunks are the opposite case: small per-cell files
    # that get opened and decoded repeatedly, live, every time a new area
    # scrolls into view. There, LZMA's slow single-threaded decode is what
    # shows up as a stutter mid-pan, and each chunk is small enough that
    # DEFLATE's 32KB window covers most of it anyway - so DEFLATE stays here.
    LARGE_SINGLE_FILE_LZMA = {'map.sqlite'}
    # map.mbtiles holds already-gzipped MVT blobs: recompressing gains ~0 and
    # only costs CPU, so it is STORED (install = plain copy, opens instantly).
    STORED = {'map.mbtiles'}
    # compresslevel applies only to ZIP_DEFLATED entries (the docs: "When
    # using ZIP_STORED or ZIP_LZMA it has no effect") - LZMA entries below
    # are unaffected by it, exactly matching the per-file split above.
    with zipfile.ZipFile(output, 'w', compresslevel=6) as z:
        for p in sorted(staging.rglob('*')):
            ...
            compress_type = (
                zipfile.ZIP_LZMA if name in LARGE_SINGLE_FILE_LZMA
                else zipfile.ZIP_STORED if name in STORED
        ...
        try:
            ...
            try:
                ...
                need = {'features','names','categories','search_fts','spatial','node_data','way_data',
                        'segments','road_index','turn_restrictions','nodes','ways','edges','places','poi'}
                if need - have:
