...
def main():
    ...
    # Keep the previous catalog and replace only countries that have a newly
    # built ABM. This prevents weekly releases from deleting existing maps.
    previous = {}
    ...
    # Scan ABM files as the source of truth.
    # If an ABM changed, rebuild only that manifest entry.
    # If its checksum is identical, keep the old entry (including chunks and patches).
    codes = sorted(p.stem for p in args.dist.glob('*.abm'))
    for code in codes:
        abm_path = args.dist / f'{code}.abm'
        old = country_map.get(code)
        current_sha = sha256_file(abm_path)

        if old and old.get('sha256') == current_sha:
            continue

        entry = build_entry(code, args.dist, names)
        if entry:
            country_map[code] = entry

    countries = sorted(country_map.values(), key=lambda x: x.get('code', ''))
