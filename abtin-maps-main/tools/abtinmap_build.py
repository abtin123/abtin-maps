...
PLACE_MIN_ZOOM = {
    "country": 3, "state": 5, "province": 5, "city": 7, "town": 10,
    "village": 12, "suburb": 12, "neighbourhood": 14, "hamlet": 13,
}
        ...
        if kind == K_WAY:
            ...
            if routable and speed > 0 and len(junc) >= 2:
                for a, b in zip(junc, junc[1:]):
                    ...
                    fwd = 0 if (attr & 2) else sec10
                    bwd = 0 if (attr & 1) else sec10
                    if fwd or bwd:
...
def main(argv=None):
    ...
    ap.add_argument("--overview-zooms", default="5,8,11")
    ...
    o.overview_tol = [500.0 if z <= 6 else 100.0 for z in o.overview_zooms]
    o.base_tol = 3.0
    build(o)
    return 0
