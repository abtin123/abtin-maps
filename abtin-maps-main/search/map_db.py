    ...
    try:
        ...
        for p in places:
            ...
            if g:
                ...
                lat = lon = None
            add(p['id'], 'place', p.get('name',''), p.get('name_fa',''), p.get('name_en',''),
                p.get('place_type','place'), lat, lon)
        for r in roads:
            if not (r.get('name') or r.get('name_fa') or r.get('name_en')):
                continue
            if r.get('name') and r.get('name') == r.get('name_fa'):
                r['name_fa'] = ''
            if r.get('name') and r.get('name') == r.get('name_en'):
                r['name_en'] = ''
            g = r.get('geometry', [])
            if not g:
                continue
            lat = sum(y for _, y in g) / len(g); lon = sum(x for x, _ in g) / len(g)
            add(r['id'], 'road', r.get('name',''), r.get('name_fa',''), r.get('name_en',''),
                r.get('road_class','road'), lat, lon, road_class=r.get('road_class','road'))
