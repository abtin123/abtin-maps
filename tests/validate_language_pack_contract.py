...
language packs:
  ...
  1. tool/l10n/source/<code>.json exists for every language in LANGUAGES,
     with exactly the same keys and the same {placeholder} tokens as
     tool/l10n/source/fa.json (the base language), and no empty values.
  ...
  2. With --require-complete: out/manifest.json lists every language,
     ...
     JSON identical to tool/l10n/source/<code>.json, and its sha256 matches
...
def check_locales() -> dict[str, dict[str, str]]:
    ...
    for code in LANGUAGES:
        ...
        if not path.exists():
            issues.append(f"{code}: tool/l10n/source/{code}.json is missing")
            continue
        try:
...
def check_build(locales: dict[str, dict[str, str]]) -> None:
    ...
    for code in LANGUAGES:
        ...
        if data != locales.get(code):
            issues.append(
                f"{code}: {pack_path.name} content does not match tool/l10n/source/{code}.json"
            )
