...
def test_build_and_validate(tmp_path, monkeypatch):
    ...
    with zipfile.ZipFile(out) as z:
        ...
        assert 'vector/index.json' in names and any(n.startswith('vector/roads/') for n in names)
...
def test_regional_split_is_geographic(tmp_path, monkeypatch):
    ...
    with zipfile.ZipFile(out_dir / 'US-NE.abm') as z:
        ...
        names = z.namelist()
        road_chunks = [n for n in names if n.startswith('vector/roads/') and n.endswith('.bin')]
        roads = b''.join(z.read(n) for n in road_chunks)
...
def test_routing_graph_is_disk_backed_and_preserves_format(tmp_path):
    ...
    assert con.execute("select name from sqlite_master where name='node_rtree'").fetchone()
    con.close()
