...
def test_indexed_vector_and_routing_members(tmp_path, monkeypatch):
    out = tmp_path / 'indexed.abm'
    monkeypatch.setattr('sys.argv', ['build_abm.py', str(SAMPLE / 'mini_country.jsonl'), '--country', 'IR', '--output', str(out)])
    main()
    with zipfile.ZipFile(out) as z:
        expected = {
            'vector/roads.idx', 'vector/buildings.idx', 'vector/landuse.idx',
            'vector/water.idx', 'vector/boundaries.idx', 'vector/places.idx',
            'routing/routing.sqlite',
        }
        assert expected.issubset(set(z.namelist()))
        for name in expected - {'routing/routing.sqlite'}:
            assert z.getinfo(name).compress_type == zipfile.ZIP_STORED
        for name in [n for n in z.namelist() if n.startswith('vector/') and n.endswith('.bin')]:
            assert z.getinfo(name).compress_type == zipfile.ZIP_STORED
        idx = z.read('vector/roads.idx')
        assert idx.startswith(b'ABMIDX1\n')
        with tempfile.NamedTemporaryFile(suffix='.sqlite') as f:
            f.write(z.read('routing/routing.sqlite')); f.flush()
            ...
            assert db.execute("SELECT 1 FROM sqlite_master WHERE type='index' AND name='idx_edges_start'").fetchone()
            db.close()
