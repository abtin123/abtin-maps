...
def create_search_db(path: Path, pois: list[dict], places: list[dict], roads: list[dict]) -> int:
    ...
    # OSM node IDs and way IDs live in separate namespaces.  A search DB
    # combines both, so blindly INSERT OR REPLACE would silently drop a place
    # when a road/way happened to have the same numeric ID.  Preserve the
    # first ID for backwards compatibility and allocate deterministic negative
    # IDs for collisions.
    used = set()
    normalized_rows = []
    next_collision = -1
    for row in rows:
        ...
        rid = int(row[0])
        if rid in used:
            while next_collision in used:
                next_collision -= 1
            row[0] = next_collision
            next_collision -= 1
        used.add(int(row[0]))
        normalized_rows.append(tuple(row))

    con.executemany("INSERT OR REPLACE INTO places VALUES (?,?,?,?,?,?,?,?)", normalized_rows)
    con.execute("INSERT INTO search_fts(search_fts) VALUES('rebuild')")
    con.executemany("INSERT OR REPLACE INTO spatial VALUES (?,?,?,?,?)",
                    [(r[0], r[6], r[6], r[5], r[5]) for r in normalized_rows])
