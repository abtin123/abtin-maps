...
The PMTiles payload remains the first declared tile-data region; only the routing graph and search database are appended after it. Styles, glyphs and sprites are application renderer assets, not per-country map data. The mobile reader therefore continues to use one `.abm` file.

## Zoom contract

- vector source zoom range: 2-16, with client overzoom to 18
...
- buildings: zoom 14
- POI: zoom 12
- vector source maximum zoom: 16; the app renderer overzooms to 18
...
5. validates PMTiles, graph, vector POI layer and offline search index; renderer styles are validated separately as app assets;
