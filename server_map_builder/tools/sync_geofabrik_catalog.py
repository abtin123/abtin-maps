...
def _country_code(properties: dict[str, Any]) -> tuple[str, str, bool] | None:
    ...
    if len(iso1) > 1:
        # Extract مشترک چند کشور: یک شناسهٔ پایدار و کشور والد برای گروه‌بندی.
...
def _normalise_generated(feature: dict[str, Any], curated: dict[str, dict[str, Any]]) -> dict[str, Any] | None:
    ...
    # هنگام وجود پروفایل دستی (مثلاً US-NE) همهٔ نام‌ها و تنظیماتِ تأییدشدهٔ
    # آن حفظ می‌شوند؛ URL رسمی index فقط جای URL قدیمی را می‌گیرد.
    ...
    entry.setdefault("render_max_zoom", 16)
...
def main() -> int:
    ...
    # اگر برای یک کشور profile منطقه‌ای تأییدشده داریم (US-NE، US-MW و ...)،
    # package کامل همان کشور نباید هم‌زمان وارد catalog شود؛ کاربر باید shardهای
    # قابل مدیریت را ببیند، نه یک فایل بسیار بزرگ تکراری. با وجود این، کشورهایی
    # که فقط یک profile کامل دارند (مثل IR) بدون تغییر باقی می‌مانند.
    sharded_parents = {
    ...
    # پروفایل دستی ممکن است یک extract مشترک یا region ویژه باشد که در index
    # بدون ISO مناسب آمده است. این‌ها را فقط زمانی نگه می‌داریم که URL رسمی
    # Geofabrik داشته باشند؛ URL ناشناخته هرگز وارد catalog خودکار نمی‌شود.
    for code, entry in curated.items():
        ...
        entries.setdefault(code, {
            **entry,
            ...
            "render_max_zoom": int(entry.get("render_max_zoom") or 16),
