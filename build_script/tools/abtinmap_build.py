...
class Extractor(osmium.SimpleHandler):
    ...
    def node(self, n):
        ...
        for k in ("amenity", "shop", "tourism", "leisure", "aeroway", "highway"):
            ...
            if v and v in POI_CLASSES:
                klass = POI_CLASSES[v]
                break
        if klass is None and t.get("enforcement") == "maxspeed":
    ...
    def way(self, w):
        ...
        if t.get("railway") in ("rail", "subway", "light_rail", "tram"):
            self.sink.emit_line(pts, KLASS_RAILWAY, 0, 10, name, 0, False, None, 0)
            return
        ...
        if wt in ("river", "stream", "canal", "drain"):
            self.sink.emit_line(pts, KLASS_WATERWAY, 0,
                                9 if wt == "river" else 13, name, 0, False, None, 0)
...
class ShardSink:
    ...
    def emit_line(self, pts, klass, attr, min_zoom, name, speed_code,
                  routable, junction_idx, speed_kmh):
        for lon, lat in pts:
            self._track(lon, lat)
            ...
            for seg, seg_junc in self._split_by_tile(geom, z, keep_junc, base, pts):
                ...
                self._write(tile_id(z, x, y), K_WAY,
                            (seg, klass, attr, min_zoom, name, speed_code,
                             routable and base, sorted(seg_junc), speed_kmh))
...
def encode_tile(tid, records, pool, opts):
    ...
    for kind, payload in records:
        if kind == K_WAY:
            seg, klass, attr, min_zoom, name, speed_code, routable, junc, speed = payload
            refs = [node_of(lon, lat) for lon, lat in seg]
            wi = len(ways)
            ways.append((klass, attr, min_zoom, pool.get(name), speed_code, refs))
            if routable and speed > 0 and len(junc) >= 2:
                for a, b in zip(junc, junc[1:]):
                    ...
                    # bit0 (=1) = onewayForward → حرکت رو به جلو ممنوع → fwd باید صفر شود
                    # bit1 (=2) = onewayBackward → حرکت به عقب ممنوع → bwd باید صفر شود
                    # نسخه‌ی قبلی این دو را برعکس خوانده بود (fwd را با بیت backward
                    # صفر می‌کرد و برعکس)، یعنی برای هر خیابان یک‌طرفه دقیقاً جهت
                    # عکسِ مجاز را باز می‌گذاشت و جهت درست را می‌بست.
                    fwd = 0 if (attr & 1) else sec10
                    bwd = 0 if (attr & 2) else sec10
                    if fwd or bwd:
    ...
    for klass, attr, min_zoom, name_id, speed_code, refs in ways:
        w_uvarint(wb, klass)
        w_uvarint(wb, attr)
...
def build(opts):
    ...
    with tempfile.TemporaryDirectory(dir=opts.tmpdir) as tmp:
        ...
        struct.pack_into("<8sHHBB6B", header, 0, MAGIC, VERSION,
                         0b111, opts.base_zoom, len(opts.overview_zooms), *ovz)
        b = sink.bbox
