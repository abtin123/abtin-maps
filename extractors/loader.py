...
def load(path: Path) -> Dataset:
    ...
    #
    # Every extractor (roads/buildings/landuse/water/boundaries/places/pois
    # in extractors/osm.py, plus routing/graph_builder.py) looks only at an
    # object's OWN tags_dict() - none of them ever pull tags down from a
    # parent relation onto an untagged member way, or turn an untagged node
    # into a record. So an object with zero tags can never produce output,
    # and it's safe (and, for a country-sized PBF where the vast majority of
    # nodes are untagged shape points on a way, essential for performance)
    # to skip building a dict for it at all.
    class Handler(osmium.SimpleHandler):
        def node(self, o):
            if not o.tags: return
            loc = o.location
            d.nodes.append({
                "id": o.id,
