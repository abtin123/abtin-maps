...
def clip_dataset(dataset: Dataset, bbox: tuple[float, float, float, float]) -> Dataset:
    ...
    A feature is copied whole when its geometry touches/crosses the region
    bbox. The builder never slices a road/way at a regional boundary. This
    makes the same OSM way (and its stable node IDs) available in every
    neighboring region it crosses, which is the basis for later cross-region
    routing/stitching.
    """
    clipped = Dataset()
