...
# "service"/"track" (driveways, parking aisles, farm tracks) are intentionally
# excluded: they are by far the most numerous way class in any OSM extract
# (routing/graph_builder.py already excludes them from the routing graph for
# the same reason) yet add almost nothing to what's visible on a phone map.
# Rendering them was the single largest contributor to .abm size - for IR
# they alone accounted for the majority of the 1.76M "roads" vector features
# (311 MiB pre-zip) and, worse, every one of them was also being written into
# map.sqlite's search index (see search/map_db.py) despite having no name.
