...
def build_entry(code: str, dist: Path, names: dict) -> dict | None:
    ...
    http_meta = {}
    candidates = [
        dist / f'source-meta-{code}.json',
        dist / f'source-meta-{code.split("-", 1)[0]}.json',
        dist / 'source-meta.json',
    ]
    for source_meta in candidates:
        if not source_meta.exists():
            continue
        try:
            http_meta = json.loads(source_meta.read_text(encoding='utf-8'))
            http_meta = {k: http_meta[k] for k in ('etag', 'last_modified', 'size') if k in http_meta}
            if http_meta:
                break
        except Exception:
            pass
