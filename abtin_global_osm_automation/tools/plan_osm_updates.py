...
def main() -> None:
    ...
    for code in sorted(published & set(countries)):
        ...
        if previous is None:
            # The map already exists (such as current IR), so establish a baseline
            # without rebuilding it on the first weekly run.
            next_state[code] = current
        elif fingerprint_changed(previous, current):
            changed.append(code)

    # Missing countries are intentionally built even if HEAD was unavailable. They
    # have no published map; build_country_package performs the definitive GET.
