...
def encode_index(
    ...
    previous_id = 0
    previous_end = 0
    # Offsets are encoded relative to the beginning of the tile-data block.
    # This makes the compressed index independent of its own final byte offset,
    # eliminating the old fixed-point/oscillation failure on large maps.
    cursor = 0
...
def build_graph(
    ...
    # The index now stores offsets relative to tile-data byte zero. The app
    # reconstructs absolute offsets as (stringsOffset + stringsComp) + relative.
    # Therefore index compression length no longer participates in a fixed-point
    # calculation and large builds cannot oscillate between two index sizes.
    index_compressed, index_raw_size = encode_index(records, compressor)
