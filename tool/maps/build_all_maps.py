    ...
    with tempfile.TemporaryDirectory(prefix="abtin-map-build-") as temp:
        for number, entry in enumerate(selected, 1):
            ...
            try:
                ...
                previous_files = [
                    str(part.get("name", ""))
                    for part in previous.get("files", [])
                    if isinstance(part, dict) and part.get("name")
                ]
                release_has_archive = archive.name in existing_assets or (
                    bool(previous_files) and all(name in existing_assets for name in previous_files)
                )
