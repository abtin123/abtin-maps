...
def main() -> int:
    ...
    parser.add_argument("--selector", default="all", help="Map selector: IR, ALL, or ALL/IR")
    ...
    if args.previous and args.previous.is_file():
        shutil.copy2(args.previous, manifest)
    else:
        empty_manifest(manifest, args.release_tag)
    try:
    ...
    # The release catalog remains complete; selector only controls which entries
    # are rebuilt in this job. This is important for commands such as IR and
    # ALL/IR: excluded maps stay available from the previous release.
    raw_selector = str(args.selector or "all").strip().upper()
    selector_parts = [p.strip() for p in raw_selector.split("/") if p.strip()]
    if not selector_parts:
        selector_parts = ["ALL"]
    if selector_parts[0] == "ALL":
        excluded = {p for p in selector_parts[1:] if p != "ALL"}
        selected_entries = [
            e for e in entries
            if str(e.get("code", "")).upper() not in excluded
            and str(e.get("country_code", "")).upper() not in excluded
        ]
    else:
        included = set(selector_parts)
        selected_entries = [
            e for e in entries
            if str(e.get("code", "")).upper() in included
            or str(e.get("country_code", "")).upper() in included
        ]
    if not selected_entries:
        raise SystemExit(f"selector matched no catalog entries: {args.selector}")
    ...
    catalog.write_text(json.dumps({"release_tag": args.release_tag, "count": len(entries), "entries": entries}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    selected = [entry for index, entry in enumerate(selected_entries) if index % args.chunk_count == args.chunk_index]
    successes: list[dict] = []
    failures: list[dict] = []
    ...
    with tempfile.TemporaryDirectory(prefix="abtin-map-build-") as temp:
        for number, entry in enumerate(selected, 1):
            ...
            try:
                ...
                if previous_signature == signature and previous.get("sha256"):
                    print(f"SKIP {code}: source signature unchanged ({signature})")
                    successes.append({"code": code, "name_fa": entry.get("name_fa", code), "name_en": entry.get("name_en", code), "archive": archive.name, "skipped": True, "source_signature": signature})
                    continue
