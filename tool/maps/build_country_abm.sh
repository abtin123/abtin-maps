...
BBOX="${4:-}"
# Accept both the documented positional bbox and the --bbox=VALUE form used by
# the catalog builder. Normalize before invoking osmium so the option is passed
# as a single --bbox=VALUE argument.
if [[ "$BBOX" == --bbox=* ]]; then
  BBOX="${BBOX#--bbox=}"
fi
CODE="${CODE^^}"
...
if [[ "$PBF_SOURCE" =~ ^https?:// ]]; then
  ...
  # Geofabrik publishes an MD5 sidecar for its PBF extracts. Verify the source
  # before spending hours in Planetiler/graph generation.
  if download_with_retry "$PBF_SOURCE.md5" "$WORK_DIR/input.osm.pbf.md5"; then
    expected_md5="$(awk '{print $1; exit}' "$WORK_DIR/input.osm.pbf.md5")"
    (cd "$WORK_DIR" && echo "${expected_md5}  input.osm.pbf" | md5sum -c -)
  else
    echo "Warning: PBF MD5 sidecar unavailable; continuing without source checksum." >&2
  fi
else
  test -f "$PBF_SOURCE"
...
python3 -c 'import osmium, zstandard, brotli' 2>/dev/null || {
  echo "Missing Python dependencies: osmium zstandard brotli. Install with: python3 -m pip install osmium zstandard brotli" >&2
  exit 2
}
...
python3 "$ROOT/tool/maps/build_search_index.py" \
  --pbf "$WORK_DIR/input.osm.pbf" \
  --output "$WORK_DIR/search.sqlite"
python3 "$ROOT/tool/maps/pack_abm_container.py" \
  --region "$CODE" \
  --pmtiles "$WORK_DIR/vector.pmtiles" \
  --graph "$WORK_DIR/graph.abm" \
  --search-index "$WORK_DIR/search.sqlite" \
