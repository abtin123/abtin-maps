...
cp "$ROOT/tool/maps/abtin_basemap.yml" "$WORK_DIR/abtin_basemap.yml"; cp "$ROOT/tool/maps/styles/day.json" "$WORK_DIR/day.json"; cp "$ROOT/tool/maps/styles/night.json" "$WORK_DIR/night.json"
mkdir -p "$WORK_DIR/resources/glyphs/Vazirmatn" "$WORK_DIR/resources/sprites"
GLYPH_DIR="$WORK_DIR/resources/glyphs/Vazirmatn"; GLYPH_RANGES=(0-255 256-511 1536-1791 1792-2047 8192-8447 64256-64511 64512-64767 65024-65279 65280-65535); LOCAL_GLYPH_DIR="$ROOT/assets/glyphs/Vazirmatn"; GLYPH_BASE_URL='https://raw.githubusercontent.com/openmaptiles/fonts/gh-pages/Klokantech%20Noto%20Sans%20Regular'
for range in "${GLYPH_RANGES[@]}"; do if [[ -s "$LOCAL_GLYPH_DIR/${range}.pbf" ]]; then cp "$LOCAL_GLYPH_DIR/${range}.pbf" "$GLYPH_DIR/${range}.pbf"; else download_with_retry "$GLYPH_BASE_URL/${range}.pbf" "$GLYPH_DIR/${range}.pbf"; fi; done
SPRITE_DIR="$ROOT/assets/sprites"; for sprite in abtin.json abtin.png abtin@2x.json abtin@2x.png; do test -f "$SPRITE_DIR/$sprite" || { echo "Missing required POI sprite: $SPRITE_DIR/$sprite" >&2; exit 2; }; cp "$SPRITE_DIR/$sprite" "$WORK_DIR/resources/sprites/"; done
resource_args=(); for glyph in "$WORK_DIR"/resources/glyphs/Vazirmatn/*.pbf; do resource_args+=(--resource "glyphs/Vazirmatn/$(basename "$glyph")=$glyph"); done; for sprite in "$WORK_DIR"/resources/sprites/*; do resource_args+=(--resource "sprites/$(basename "$sprite")=$sprite"); done
