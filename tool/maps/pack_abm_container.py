...
def build(a):
 ...
 resources={}
 missing=[k for k in REQ if k not in resources and k != 'search/places.sqlite']
 ...
 # Renderer styles, glyphs and POI sprites are app-bundled. They are never
 # copied into country ABMs, keeping each map data-only.
 for name in sorted(REQ):
  if name=='search/places.sqlite': continue
  payloads.append((name,resources[name].read_bytes(),'none',0))
 ...
 for name,data,codec,raw_length in payloads:
  ...
  cursor+=len(data)
 meta=dict(base_meta)
 meta['abtin_container']={'version':4,'region':a.region.upper(),'contracts':{'pmtiles_minzoom':2,'pmtiles_maxzoom':16,'app_overzoom_maxzoom':20,'offline_search':'search/places.sqlite','rendered_pois':True,'styles':'app_bundled','glyphs':'app_bundled','sprites':'app_bundled'},'graph':graph_info,'entries':entries}
...
def main():
 p=argparse.ArgumentParser(description=__doc__); p.add_argument('--region',required=True); p.add_argument('--pmtiles',type=Path,required=True); p.add_argument('--graph',type=Path,required=True); p.add_argument('--search-index',type=Path,required=True); p.add_argument('--day-style',type=Path); p.add_argument('--night-style',type=Path); p.add_argument('--resource',action='append',default=[],help='Ignored compatibility option; renderer assets are app-bundled'); p.add_argument('--output',type=Path,required=True); build(p.parse_args())
