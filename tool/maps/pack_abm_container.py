...
def build_container(args: argparse.Namespace) -> None:
    ...
    if not args.search_index.is_file():
        raise SystemExit(f"Search index is missing: {args.search_index}")

    entry_sources: list[tuple[str, Path]] = [
        ("styles/day.json", args.day_style),
        ("styles/night.json", args.night_style),
        ("search/places.sqlite", args.search_index),
        *args.resource,
    ]
    ...
    metadata["abtin_container"] = {
        ...
        "contracts": {"pmtiles_minzoom": 2, "pmtiles_maxzoom": 20, "poi_minzoom": 12, "building_minzoom": 14, "major_road_label_minzoom": 8, "local_road_label_minzoom": 14, "offline_search": "search/places.sqlite"},
        "graph": graph_info,
        "entries": entries,
    }
