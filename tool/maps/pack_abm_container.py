...
def build_container(args: argparse.Namespace) -> None:
    ...
    metadata.pop("abtin_container", None)
    # Metadata length affects tile_data_offset, and offsets inside abtin_container
    # affect metadata length. Iterate until decimal offsets become stable.
    for _ in range(12):
        root_offset = HEADER_SIZE
        metadata_offset = root_offset + len(parts.root_directory)
        previous = metadata.get("abtin_container")
        encoded = encode_internal(metadata_bytes(metadata), parts.internal_compression)
        leaf_offset = metadata_offset + len(encoded)
        ...
        if previous == metadata["abtin_container"]:
            break
    else:
        raise SystemExit("Could not stabilize container metadata offsets.")
    ...
    root_offset = HEADER_SIZE
    metadata_offset = root_offset + len(parts.root_directory)
    leaf_offset = metadata_offset + len(encoded_metadata)
    tile_offset = leaf_offset + len(parts.leaf_directories)
    total_tile_data = len(parts.tile_data) + sum(len(payload) for _, payload in payloads)
    final_length = tile_offset + total_tile_data
    # One final check that metadata offsets were calculated from the final length.
    container = metadata["abtin_container"]
    assert isinstance(container, dict)
    graph = container["graph"]
    assert isinstance(graph, dict)
    if graph["offset"] != tile_offset + len(parts.tile_data):
        raise SystemExit("Container metadata offset does not match PMTiles layout.")
    ...
    if tile_offset + total_tile_data != final_length:
