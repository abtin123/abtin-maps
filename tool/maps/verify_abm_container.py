...
REQUIRED_ENTRIES = {"styles/day.json", "styles/night.json"}
...
def verify(path: Path, expected_region: str | None) -> dict[str, object]:
    ...
    with path.open("rb") as handle:
        ...
        if not isinstance(region, str) or len(region) != 2 or region != region.upper():
        ...
        for entry in entries:
            ...
            offset, length = verify_payload(
                ...
                expected_path=internal_path if internal_path in REQUIRED_ENTRIES else None,
            )
...
def main() -> None:
    ...
    parser.add_argument("--region", help="Expected two-letter uppercase region code")
